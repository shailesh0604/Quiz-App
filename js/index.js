    ...
    answers.forEach((answer, index) => {
        answersHTML += `
            <div class="quiz-answer">
                <input type="radio" class="radio-ans-btn" id="radio${index + 1}" name="answer" value="${answer}" />
                <label class="radio-label" for="radio${index + 1}">
                    <span class ="radio-inner-circle">${String.fromCharCode(65 + index)}</span>
        ...
        <div class ="quiz-header">
                <div class ="question-number d-flex align-items-center gap-2">
                    ...
                    <div class="question-counting">
                        <span id="currentNumber">${currentIndex + 1}</span> / <span id="totalNumber">${maxQuestion}</span>
                    </div>
                </div>
        ...
        document.querySelectorAll('input[name="answer"]').forEach((input) => {
            ...
            if (input.value.trim() === selectedValue) {
                ...
                if (input.value.trim() === correctAnswer) {
                    ...
                    input.nextElementSibling.innerHTML += '<div class="right-icon"><i class="fa-solid fa-circle-check fa-lg" style="color: #ffffff;"></i></div>';

                } else {
                    input.parentElement.style.backgroundColor = "rgb(255 139 152)"; // red
                    input.nextElementSibling.innerHTML += '<div class="wrong-icon"><i class="fa-regular fa-circle-xmark fa-lg" style="color: #ffffff;"></i></div>';
                }
